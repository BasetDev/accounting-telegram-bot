...
🗑 حذف [delete_debt:{txn_id}] / [delete_receivable:{txn_id}]
    └── Confirmation (inline)
        ├── ✅ تأیید  [confirm_yes] → Delete (cascade: payments + photo cleanup) → Main Menu
        └── ❌ رد     [confirm_no]  → Cancel → Main Menu
...
══════════════════════════════════════════════════════════════
MEDIA MANAGEMENT & UPLOAD DIRECTORY
══════════════════════════════════════════════════════════════

Upload Directory: uploads/ (configured via UPLOAD_DIR in .env)

Photo Naming Convention:
├── {user_id}_{YYYYMMDD}_{HHMMSS}_{8-char-suffix}.jpg
├── Example: 1234567890_14050503_162525_DeAADPQQ.jpg
└── Generated by _save_photo() in main_handler.py

Database References:
├── transactions.photo_path → Absolute path to transaction receipt photo
└── payments.photo_path     → Absolute path to payment receipt photo

Photo Lifecycle:
├── Upload: _save_photo() downloads from Telegram → saves to uploads/ → stores absolute path in DB
├── Edit:   Old photo deleted from disk → new photo saved → DB updated
├── Delete: TransactionRepository.delete() / CustomerRepository.delete()
│           → collects photo paths from DB records
│           → deletes DB records first
│           → deletes photo files from filesystem via _cleanup_photo_file()
└── View:   view_photo:{txn_id} / view_payment_photo:{txn_id}
            → reads photo_path from DB → sends FSInputFile to Telegram

Backup & Restore:
├── Backup: _add_media_to_zip() walks uploads/ → adds all files to ZIP as media/*
├──         Photo paths normalized to relative (just filename) during backup
├── Restore: _extract_media_from_zip() extracts media/* → uploads/
├──          Photo paths remapped from relative to absolute during restore
└──          Existing uploads/ files are NOT cleaned up before restore (by design)

Orphan Detection:
└── scripts/detect_orphan_media.py
    ├── python scripts/detect_orphan_media.py              # Report only
    ├── python scripts/detect_orphan_media.py --delete     # Report and delete orphans
    └── Cross-references DB photo_path fields vs. files on disk
